...
namespace SplinePower.Editors
{
    ...
    public class SplineFormerEditor : Editor
    {
        static public class Exporter
        {
            ...
            public static void MeshToAsset(Mesh mesh, string filename, SplineFormer.ExportOptionsContainer options)
            {
                try
                {
                    ...
                    {
                        path = AssetsDirectory + options.DefaultExportFolder + "/" + filename;
                    }
        ...
        private bool _showWaypointsModule;
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (var group in groups)
            {
                ...
                group.SegmentMesh =
                    (Mesh)
                        EditorGUILayout.ObjectField(group.SegmentMesh, typeof (Mesh), false,
                            GUILayout.MinWidth(_fieldWidth));
                group.MeshFilter =
                    EditorGUILayout.ObjectField(group.MeshFilter, typeof (MeshFilter), true,
                        GUILayout.MinWidth(_fieldWidth))
                        as MeshFilter;
                group.MeshCollider =
                    EditorGUILayout.ObjectField(group.MeshCollider, typeof (MeshCollider), true,
                        GUILayout.MinWidth(_fieldWidth)) as MeshCollider;
                EditorGUILayout.EndHorizontal();
                ...
                if (FoldoutOption("Caps", group))
                {
                    ...
                    group.StartPiece =
                        (Mesh)
                            EditorGUILayout.ObjectField(group.StartPiece, typeof (Mesh), false,
                                GUILayout.MinWidth(_fieldWidth * 2));
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("End Piece");
                    group.EndPiece =
                        (Mesh)
                            EditorGUILayout.ObjectField(group.EndPiece, typeof (Mesh), true,
                                GUILayout.MinWidth(_fieldWidth * 2));
                    EditorGUILayout.EndHorizontal();
                }
                //====================
                ...
                {
                    group.ProcessOriginNormals = EditorGUILayout.ToggleLeft("Process Origin Normals",
                        group.ProcessOriginNormals);
                    ...
                    group.ProcessOriginTangents = EditorGUILayout.ToggleLeft("Process Origin Tangents",
                        group.ProcessOriginTangents);
                    ...
                    group.RecalculateNormals = EditorGUILayout.ToggleLeft("Recalculate Normals",
                        group.RecalculateNormals);
            ...
            EditorGUI.BeginChangeCheck();
            _splineFormer.SegmentsNumber = Mathf.Max(1,
                EditorGUILayout.IntField("Segments Count", _splineFormer.SegmentsNumber));
            _splineFormer.Coefficient = Mathf.Max(0.01f,
                EditorGUILayout.FloatField("Coefficient", _splineFormer.Coefficient));
            ...
            _splineFormer.EndTangentsMode =
                (SplineFormer.EndTangentsModes)
                    EditorGUILayout.EnumPopup("End Tangents Mode", _splineFormer.EndTangentsMode);
            EditorGUILayout.Separator();
            ...
            _showWaypointsModule = EditorGUILayout.Foldout(_showWaypointsModule, "Waypoints");
            if (_showWaypointsModule)
            {
                EditWaypointsModule();
            }
        ...
        private void SetSceneDirty()
        {
            ...
            EditorUtility.SetDirty(_splineFormer);
        ...
        private void EditWaypointsModule()
        {
            var module = _splineFormer.WaypointsModule;
            EditorGUI.BeginChangeCheck();
            module.Enabled = EditorGUILayout.Toggle("Enabled", module.Enabled);

            if (module.Enabled)
            {
                module.WaypointsNumber = EditorGUILayout.IntField("Number", module.WaypointsNumber);
                module.DrawPath = EditorGUILayout.Toggle("Draw Path", module.DrawPath);
                module.Shift = EditorGUILayout.Vector3Field("Shift", module.Shift);
                if (_splineFormer.Loop)
                {
                    EditorGUI.BeginDisabledGroup(true);
                    EditorGUILayout.Toggle("Loop", true);
                    EditorGUI.EndDisabledGroup();
                }
                else
                {
                    module.Loop = EditorGUILayout.Toggle("Loop", module.Loop);
                }
            }
        ...
        private void EditVisualOptions()
        {
            EditorGUI.BeginChangeCheck();
        ...
        private void EditExportOptions()
        {
            ...
            EditorGUILayout.LabelField("Default Export Folder", _splineFormer.ExportOptions.DefaultExportFolder);

            if (GUILayout.Button("Select"))
            {
                var defaultFolder = _splineFormer.ExportOptions.DefaultExportFolder;
                ...
                {
                    _splineFormer.ExportOptions.DefaultExportFolder = selectedFolder.Replace(assetsFolder, "");
                }
            }
            EditorGUILayout.EndHorizontal();
