...
public class SplineFormer : MonoBehaviour
{
    ...
    [HideInInspector] public List<SplineNode> Nodes = new List<SplineNode>();

    public int SegmentsNumber;
    public bool QuadraticSmooth = true;
    public bool RollerCoasterFix = false;

    public enum EndTangentsModes
    {
        Clamp,
        Balanced
    }

    public EndTangentsModes EndTangentsMode;

    [SerializeField] public ExportOptionsContainer ExportOptions;
    [SerializeField] public VisualOptionsContainer VisualOptions;
    [SerializeField] public WaypointsModule WaypointsModule;
    ...
    public SplineNode EndNode { get { return _endNode; } }
    ...
    public SplineNode StartNode { get { return _startNode; } }
    ...
    public float GetNodeT(float fullT, out int index)
    {
        float lenght = 0f;
        var node = _startNode;
        var maxDist = fullT * _splineLength;
        while (!Object.ReferenceEquals(node.RightNeighbor, null))
        {
            if (node.RightNeighbor.DistanceFromStart > maxDist)
            {
                index = node.Number;
                return (maxDist - node.DistanceFromStart) / node.DistanceToNext;
            }
    ...
    /// <summary>
    /// Fixes any accidents and does some preparations
    /// </summary>
    private void Fix()
    {
        RefreshNodesValues();
    ...
    private void FullMeshRebuild()
    {
        RecalculateValues();
        RebuildAllGroups();
        WaypointsModule.RecalculateWaypoints(this);
    }
    ...
    public Vector3[] GetSplinePoints(int segmentsNumber)
    {
        ...
        for (int s = 0; s < segmentsNumber; s++)
        {
            ...
            points[s] = ltw * points[s];
        }

        return points;
    }

    private void DrawSegmentsPath()
    {
        Gizmos.color = Color.red;
        Vector3[] points = GetSplinePoints(SegmentsNumber);
        for (int s = 1; s < points.Length; s++)
        {
            Gizmos.DrawLine(points[s - 1], points[s]);
        }
    }
    ...
    private void Initialize()
    {
        ...
        if (WaypointsModule == null) WaypointsModule = new WaypointsModule();
    }
    ...
    private void RebuildMesh(LoftingGroup group)
    {
        ...
        for (int s = 0; s < group.ActiveSegmentsNumber; s++)
        {
            ...
            for (int i = 0; i < source.vertexCount; i++)
            {
                ...
                else if (nodeIndex > nodesCount - 1)
                {
                ...
                if (nodeIndex >= nodesCount - 1)
                {
                    ...
                    {
                        nodeIndex = nodesCount - 2;
                        nextIndex = nodesCount - 1;
                    }
                }
                ...
                Quaternion directionRot =
                    Quaternion.LookRotation(dirR, upwardVector) *
                    Quaternion.Inverse(Quaternion.LookRotation(dirS, upwardVector));
    ...
    public class LoftingGroup
    {
        ...
        [NonSerialized] public Mesh ResultMesh;
        ...
        [NonSerialized] public ManagedMesh mSegmentMesh;
        public ManagedMesh mStartPiece;
        public ManagedMesh mEndPiece;

        [NonSerialized] public ManagedMesh[] ResultSegments;
    ...
    {
        public string DefaultExportFolder = "Generated/Splines";
        ...
        [SerializeField] private int _uk;

        public string GetName(SplineFormer splineFormer, LoftingGroup group)
        {
            if (!ExtendedNaming)
            {
                return String.Format("{0} Lg#{1} result", splineFormer.gameObject.name,
                    splineFormer.LoftingGroups.IndexOf(group));
            }
